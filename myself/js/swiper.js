...
const swiper = ({
  ...
  wrap.addEventListener('touchmove', (ev) => {
    ...
    if (isDir[dir]) {
      if (backOut == 'none') {
        ...
        targetEle[dir] =
          targetEle[dir] < minDistance[dir] ? minDistance[dir] : targetEle[dir];
      ...
      css(bar, {
        ['translate' + dir.toUpperCase()]: -targetEle[dir] * scale,
        opacity: 0.6,
      });
  ...
  wrap.addEventListener('touchend', (ev) => {
    ...
    let target = Math.round(
      buffer + css(scroll, 'translate' + dir.toUpperCase())
    );

    if (target > 0) {
      if (target > 40) {
        //现在到顶部了，并且已经超过顶部50px了，这个时候一松手就可以刷新了
...
const gesTure = ({ el, start, change, end }) => {
  let isGesture = false;
  (startPointDis = 0), (startPointDeg = 0), (lastScale = 1), (lastRotate = 0);
